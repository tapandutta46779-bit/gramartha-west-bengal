...
| PDF download resilience | IMPLEMENTED + VERIFIED | A normal same-page attachment download avoids blank popup tabs and browser-sensitive blob handling. Before download, the browser restores its unchanged canonical decision to survive a deployment restart without rerunning the analysis. GET and fallback POST return `application/pdf`, attachment filenames, explicit lengths, no-store and nosniff headers. |
