...
function compactCompetitionPanel(p){const c=p.competition||{};return `<article><h3>OSM competition and nearby context</h3>${competitionDiscoveryNotice(p)}${plainRow("Direct candidates inside radius",c.direct_count??"Not mapped")}${plainRow("Indirect candidates inside radius",c.indirect_count??"Not mapped")}${plainRow("Mapped candidates without a public name",(c.direct_unnamed_count||0)+(c.indirect_unnamed_count||0))}${nearestCompetitorRow("Nearest named direct",c.likely_direct_competitors?.[0])}${nearestCompetitorRow("Nearest named indirect",c.likely_indirect_competitors?.[0])}${plainRow("Competition intensity",friendlyCompetitionIntensity(c.competition_intensity))}${plainRow("Nearest market",p.catchment?.nearest_market?.name||"Not mapped in bounded scan")}${plainRow("Nearest institution",p.sector_intelligence?.institutional_buyer_candidates?.[0]?.name||"Not mapped in bounded scan")}<p class="caption">Unnamed mapped features remain in density counts but are not shown as identifiable competitors. ${safe(c.caveat||"No coordinate-backed scan was possible.")}</p></article>`;}
...
function competitionDiscoveryNotice(p){const c=p.competition||{};const shown=(c.likely_direct_competitors?.length||0)+(c.likely_indirect_competitors?.length||0);const total=(c.direct_count||0)+(c.indirect_count||0);const provenance=`<small class="osm-provenance">Scan center: ${safe(friendlyCode(c.coordinate_quality||p.catchment?.center?.coordinate_quality||"Unknown"))} · OSM index: ${safe(c.osm_data_extracted_at||"vintage unknown")}</small>`;if(total>0&&shown>0)return`<div class="reserve-note"><strong>OSM candidates found inside your radius.</strong> Identifiable names and distances are shown below; unnamed mapped features remain only in the counts.${provenance}</div>`;if(total>0)return`<div class="reserve-note"><strong>${total} relevant OSM features are mapped inside the radius, but none has a defensible public name.</strong> They remain in density counts and are not presented as identified businesses.${provenance}</div>`;if(shown>0)return`<div class="reserve-note"><strong>No OSM competitor is mapped inside ${number(p.catchment?.radius_km)} km.</strong> GramArtha found ${shown} named alternatives in the wider ${number(c.competitor_discovery_radius_km)} km discovery ring and labels them outside your planning radius.${provenance}</div>`;return`<div class="reserve-note"><strong>No named OSM candidate was found in the bounded scan.</strong> This is missing map evidence, not proof that no competitor exists.${provenance}</div>`;}
